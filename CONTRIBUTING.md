...
Thanks for helping improve the NDE Schema.org Application Profile (SCHEMA-AP-NDE).
...
So the default answer to “should we add this?” is a careful no.
The bar for adding a class or property is high, and the sections below describe how we decide.
...
A proposed class or property is assessed against the following criteria.
None is a sole gate; together they decide whether something earns a place in the profile.
...
We follow Schema.org’s documented domain and range, even if they are advisory and Schema.org’s semantics are weak, so that profile data also validates against Schema.org’s own type expectations and behaves predictably in consumers’ Schema.org tooling.
...
**Optional does not mean free:** a property a publisher can ignore is still one that every *consumer* has to learn, map and decide how to handle. 
Breadth therefore has a real cost even when nothing is required, and “it’s optional, so it does no harm” is not a sufficient argument for inclusion.
